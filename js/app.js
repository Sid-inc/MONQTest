...
for (let i = 0; i < elements.length; i++) {
  (function (element) {
    ...
    if (element.value) {
      ...
      let counterShow = -499;
      let intervalShow = setInterval(function() {
        counterShow+=10;
        document.getElementById("alert").style.transform = `translateX(${counterShow}px)`;
        if (counterShow >= 0) {clearInterval(intervalShow)};
      }, 1);
    }
